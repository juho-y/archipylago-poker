...
model = OpenAIModel("gpt-4o-mini", api_key="sk-1234", base_url="http://localhost:4000")

ai_player = Agent(
    model=model,
    system_prompt="""You are a Texas Hold'em poker player. You are playing against a human player. 
...
def get_past_game_examples() -> str:
    ...
    return fetch_past_games(min_score=8)
...
judge_agent = Agent(
    ...
    system_prompt="""
    You are a expert Texas Hold'em poker commentator with a deep understanding of the game.
    You will be presented a poker game actions and your job is to give an analysis of the game.
    Rate the players (referred as you in the game stages) actions from 1 to 10 where 1 is the worst and 10 is the best. 

    Give a justifications for your rating.
    """,
    result_type=PostGameAnalysisResult,
)
...
class PostGameAnalysis(BaseNode[PokerState, None, None]):
    async def run(self, ctx: GraphRunContext[PokerState]) -> End:
        ...
        analysis = await judge_agent.run(f"Game stages:\n{all_stages}\n\n")

        print(f"AI score: {analysis.data.score}")
        print(analysis.data.justifications)

        save_game_data(
            stages=all_stages,
            score=analysis.data.score,
            justifications=analysis.data.justifications,
        )
